...
using GishnizApp.Framework.Extensions;
...
public class CustomExceptionHandlerMiddleware
{
    ...
    public async Task Invoke(HttpContext httpContext, ILogger<CustomExceptionHandlerMiddleware> logger)
    {
        ...
        catch (Exception e) when (e is SecurityTokenExpiredException ex)
        {
            logger.LogError(ex, "Unauthorized exception occurred");
            await httpContext.Response.WriteSecurityTokenExpiredErrorsAsync(ex);
        }
        catch (Exception e) when (e is UnauthorizedAccessException ex)
        {
            logger.LogError(ex, "Unauthorized exception occurred");
            await httpContext.Response.WriteUnauthorizedErrorsAsync(ex);
        }
